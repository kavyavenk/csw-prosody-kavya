#!/usr/bin/env python3
"""
Generate SEAME prosody analysis report comparing to Spanish-English findings.
"""
...
def generate_report(cs_vs_en_file, cs_vs_zh_file, output_file):
    ...
    if not pathlib.Path(cs_vs_zh_file).exists():
        print(f"ERROR: Contrast file not found: {cs_vs_zh_file}")
        return
    
    print(f"Reading contrast results...")
    df_en = pd.read_csv(cs_vs_en_file)
    df_zh = pd.read_csv(cs_vs_zh_file)
    
    # Categorize features
    df_en['category'] = df_en['feature'].apply(categorize_feature)
    df_zh['category'] = df_zh['feature'].apply(categorize_feature)
    
    # Filter to significant features
    sig_en = df_en[df_en['sig'] == True].copy()
    sig_zh = df_zh[df_zh['sig'] == True].copy()
    ...
    stats_zh = calc_category_stats(df_zh, sig_zh)
    
    # Generate report
    report_lines = []
    report_lines.append("# SEAME Prosody Analysis Report")
    report_lines.append("## Replication of Spanish-English CSW Prosody Findings\n")
    
    report_lines.append("### Methodology")
    report_lines.append("- **Corpus**: SEAME (Mandarin-English)")
    ...
    report_lines.append("- **Comparisons**: CSW vs. monolingual Mandarin, CSW vs. monolingual English\n")
    
    # Get actual data summary from feature file
    feat_file = pathlib.Path("features/seame_disvoice_utt.csv")
    if feat_file.exists():
        feat_df = pd.read_csv(feat_file)
        total_utt = len(feat_df)
        cs_count = (feat_df['condition'] == 'code_switched').sum()
        zh_count = (feat_df['condition'] == 'monolingual_ZH').sum()
        en_count = (feat_df['condition'] == 'monolingual_EN').sum()
        cs_pct = 100 * cs_count / total_utt
        zh_pct = 100 * zh_count / total_utt
    ...
    report_lines.append(f"- **Monolingual Mandarin (ZH)**: {zh_count:,} ({zh_pct:.1f}%)")
    report_lines.append(f"- **Monolingual English (EN)**: {en_count:,} ({en_pct:.1f}%)")
    report_lines.append(f"- **Note**: 57,832 utterances skipped (audio files not found)\n")
    ...
    # CSW vs ZH
    report_lines.append("#### CSW vs. Monolingual Mandarin")
    for category in ['pitch', 'energy', 'duration', 'voice_quality']:
        stats = stats_zh.get(category, {})
        if stats.get('total', 0) > 0:
            report_lines.append(f"**{category.capitalize()} Features**:")
            report_lines.append(f"- Total: {stats['total']}")
            report_lines.append(f"- Significantly different: {stats['significant']} ({stats['pct_significant']:.1f}%)")
    ...
    total_sig_zh = len(sig_zh)
    pct_en = 100 * total_sig_en / len(df_en)
    pct_zh = 100 * total_sig_zh / len(df_zh)
    ...
    report_lines.append(f"- **CSW vs. ZH**: {total_sig_zh} / {len(df_zh)} features ({pct_zh:.1f}%) significantly different")
    report_lines.append("")
    ...
    report_lines.append("| Feature Category | Spanish-English | SEAME (CS vs EN) | SEAME (CS vs ZH) | Pattern Match |")
    report_lines.append("|----------------|-----------------|------------------|------------------|---------------|")
    
    for category in ['duration', 'energy', 'pitch']:
        se_pct = se_findings[category]['pct_sig']
        seame_en_pct = stats_en.get(category, {}).get('pct_significant', 0)
        seame_zh_pct = stats_zh.get(category, {}).get('pct_significant', 0)
        
        # Determine if pattern matches
        if category == 'pitch':
            match_en = "✓" if seame_en_pct < 50 else "✗"
            match_zh = "✓" if seame_zh_pct < 50 else "✗"
        else:
            match_en = "✓" if abs(seame_en_pct - se_pct) < 30 else "✗"
            match_zh = "✓" if abs(seame_zh_pct - se_pct) < 30 else "✗"
        
        report_lines.append(f"| {category.capitalize()} | {se_pct:.1f}% | {seame_en_pct:.1f}% | {seame_zh_pct:.1f}% | EN:{match_en} ZH:{match_zh} |")
    ...
    report_lines.append("| Feature | Category | p (adjusted) | Cohen's d |")
    report_lines.append("|---------|----------|--------------|-----------|")
    top_zh = sig_zh.nlargest(5, 'cohen_d', keep='all')
    ...
    report_lines.append(f"2. CSW differs prosodically from monolingual Mandarin: {pct_zh:.1f}% of features significant")
    
    if pct_en > pct_zh:
        report_lines.append("3. CSW shows **greater differences** from monolingual English than Mandarin")
    else:
        report_lines.append("3. CSW shows **greater differences** from monolingual Mandarin than English")
    
    report_lines.append("")
    report_lines.append("**Key Observations:**")
    report_lines.append("- Successfully extracted DisVoice 103 prosodic features for 52,313 utterances")
    ...
    report_lines.append("- Note: 57,832 utterances skipped due to missing audio files")
    
    report_lines.append("")
    report_lines.append("**Next Steps:**")
    report_lines.append("1. Analyze speaker-level variation")
    report_lines.append("2. Investigate language-specific effects (tonal vs. non-tonal)")
    report_lines.append("3. Compare findings with MASAC (Hindi-English) corpus")
    ...
    print(f"  - CSW vs ZH: {total_sig_zh}/{len(df_zh)} features significant ({pct_zh:.1f}%)")

if __name__ == '__main__':
    p = argparse.ArgumentParser(description='Generate SEAME prosody analysis report')
    p.add_argument('--cs-vs-en', type=str,
                   default='features/first_contrast_seame_CS_vs_EN.csv',
                   help='CSW vs EN contrast results')
    p.add_argument('--cs-vs-zh', type=str,
                   default='features/first_contrast_seame_CS_vs_ZH.csv',
                   help='CSW vs ZH contrast results')
    p.add_argument('--output', type=str,
                   default='reports/seame_prosody_analysis.md',
